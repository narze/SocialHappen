...
class Invite_pending_model_test extends CI_Controller {
	...
	function add_test(){
		...
		$campaign_id = 1;
		$page_id = 1;
		$result = $this->invite_pending->add($user_facebook_id, $campaign_id, $page_id, $invite_key);
		...
		$page_id = '1';
		$result = $this->invite_pending->add($user_facebook_id, $campaign_id, $page_id, $invite_key);
		$this->unit->run($result, 'is_string', 'add with dup user_facebook_id and page_id', $result);
	}
	...
	function add_fail_test(){
		...
		$page_id = 0;
		$result = $this->invite_pending->add($user_facebook_id, $campaign_id, $page_id, $invite_key);
		$this->unit->run($result, FALSE, 'add without page_id', $result);
	...
	function get_by_user_facebook_id_and_campaign_id_test(){
		...
		$this->unit->run($result['page_id'], 1, 'get_by_user_facebook_id_and_campaign_id_test', $result['page_id']);
	...
	function get_by_user_facebook_id_and_page_id_test(){
		$user_facebook_id = 123;
		$page_id = '1';
		$result = $this->invite_pending->get_by_user_facebook_id_and_page_id($user_facebook_id, $page_id);
		$this->unit->run(count($result) === 2, TRUE, 'get_by_user_facebook_id_and_page_id', count($result));
	...
	function get_by_user_facebook_id_and_page_id_fail_test(){
		$user_facebook_id = 555;
		$page_id = 1;
		$result = $this->invite_pending->get_by_user_facebook_id_and_page_id($user_facebook_id, $page_id);
		$this->unit->run(count($result) === 0, TRUE, 'get_by_user_facebook_id_and_page_id', count($result));
	}
