...
class Page extends CI_Controller {
	...
	function disable_facebook_bar($page_id = NULL){
		if(!$this->socialhappen->check_admin(array('page_id' => $page_id),array('role_page_edit','role_all_company_pages_edit'))){
			//no access
		} else {
			$this->load->model('page_model');
			$result = $this->page_model->update_page_profile_by_page_id($page_id, array('enable_facebook_tab_bar' => 0));
			$this->load->vars(array(
				'partial' => 'facebook-page-information',
				'page' => $page = $this->page_model->get_page_profile_by_page_id($page_id),
				'page_facebook' => $this->facebook->get_page_info($page['facebook_page_id'])
	...
	function enable_facebook_bar($page_id = NULL){
		if(!$this->socialhappen->check_admin(array('page_id' => $page_id),array('role_page_edit','role_all_company_pages_edit'))){
			//no access
		} else {
			$this->load->model('page_model');
			$result = $this->page_model->update_page_profile_by_page_id($page_id, array('enable_facebook_tab_bar' => 1));
			$this->load->vars(array(
				'partial' => 'facebook-page-information',
				'page' => $page = $this->page_model->get_page_profile_by_page_id($page_id),
				'page_facebook' => $this->facebook->get_page_info($page['facebook_page_id'])
