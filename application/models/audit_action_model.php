...
class Audit_action_model extends CI_Model {
	...
	function create_index(){
		return $this->collection->deleteIndexes()
			&& $this->collection->ensureIndex(array('app_id' => 1, 'action_id' => 1));
	}
	...
	function delete_action($app_id = NULL, $action_id = NULL){
		...
		if($check_args){
			...
			$this->collection->remove($criteria, array('$atomic' => TRUE));
			return TRUE;
		}else{
			return FALSE;
		}
	...
	function get_action_list(){
		$res = $this->collection->find()->sort(array('action_id' => 1));
		$result = array();
	...
	function get_action($app_id = NULL, $action_id = NULL){
		if(isset($app_id)){
			...
			$res = $this->collection->find($criteria)->sort(array('action_id' => 1));
			$result = array();
			foreach ($res as $entry) {
				$result[] = $entry;
			}
