 ...
 *   	},
 *   	start_timestamp : <start_timestamp to announce reward, redeem expiration date>
 *   	end_timestamp : <end_timestamp to announce reward, redeem expiration date>
 *   }]
 */
 class Reward_model extends CI_Model {
	...
	function add($input = array()){
		$check_args = (isset($input['campaign_id']) || isset($input['app_install_id']) || isset($input['page_id'])) && isset($input['start_timestamp']) && isset($input['end_timestamp']);
		if(!$check_args){
			return FALSE;
		} else {
	 ...
	 * @author Manassarn M.
	 */
	function update($reward_id = NULL, $input = NULL){
		if(!$reward_id || !$input){
			return FALSE;
		}
		$update['$set'] = array();
		...
		if(isset($input['campaign_id'])){
			...
			$criteria = 'app_install_id';
		} else if (isset($input['page_id'])){
			$criteria = 'page_id';
		}
		if(isset($criteria)){
			$update['$set']['criteria'] = array();
			$update['$set']['criteria'] = array($criteria => $input[$criteria]);
		}
		try	{
			return $this->reward->update(array('_id' => new MongoId($reward_id)),
