...
class DB_Sync {
	...
	function create_tables(){
		...
		$tables = array_map(array($this->CI->db,'dbprefix'), $tables);
		
		foreach ($tables as $table){
			$table = str_replace($this->CI->db->dbprefix,'',$table);
			$this->CI->dbforge->add_field($fields[$table]);
			foreach ($keys[$table] as $primary_key){
				$this->CI->dbforge->add_key($primary_key, TRUE);
			}
			if($this->CI->dbforge->create_table($table, TRUE)){
	...
	function special_cases_after_create(){
		$this->CI->db->query("CREATE UNIQUE INDEX user_facebook_id ON ".$this->CI->db->dbprefix('user')." (user_facebook_id)");
		$this->CI->db->query("CREATE INDEX last_activity_idx ON ".$this->CI->db->dbprefix('sessions')." (last_activity)");
	}
	...
	function insert_test_data(){
		$app = array(
			...
			array(
				...
				'app_url' => 'https://apps.socialhappen.com/beta/mockapp?app_install_id={app_install_id}', 
				'app_install_url' => 'https://apps.socialhappen.com/beta/mockapp/port/install_unit/?company_id={company_id}&user_id={user_id}&page_id={page_id}', 
				'app_install_page_url' => 'https://apps.socialhappen.com/beta/mockapp/port/install_to_page/?app_install_id={app_install_id}&user_id={user_id}&page_id={page_id}&force=1', 
				'app_config_url' => 'https://apps.socialhappen.com/beta/mockapp/admin/?app_install_id={app_install_id}&user_id={user_id}&app_install_secret_key={app_install_secret_key}',
				'app_config_facebook_canvas_path' => NULL,
				'app_support_page_tab' => 1, 
				'app_icon' =>  'https://apps.socialhappen.com/beta/mockapp/images/app_image_16.png',
				'app_image' =>  'https://apps.socialhappen.com/beta/mockapp/images/app_image_o.png',
