...
function get_page_installed(page_id) {
	$.ajax({
		async:false,
...
function show_installed_page_in_company() {
	jQuery.ajax({
		...
		success: function(json) {
			...
			refresh_installed_page_panel();
			if(json.length>0)
				view_page_app_nochange_right(json[0].page_id,json[0].facebook_page_id,json[0].page_name);
	...
	jQuery.ajax({
		...
		success: function(json) {
			...
			ul_element.append('<li><p><img class="app-image" src="'+imgsize('','normal')+'" alt="" width="64" height="64" />'
				...
				+'</span>'
				+'</p><p class="appname">'+ 'Signup Form' +'</p><input type="hidden" class="app_install_id" value="'+0+'" /></li>');
