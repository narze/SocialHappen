$(function(){	
	function get_campaign_users(page_index, jq){
		set_loading();
		$.getJSON(base_url+'campaign/json_get_users/'+campaign_id+'/'+per_page+'/'+(page_index * per_page),function(json){
			...
			if(json.length == 0) {
				...
				for(i in json){
					...
					campaign_list.find('p.thumb img').attr('src', imgsize(json[i].user_image,'square')).addClass('user-image');
					campaign_list.find('h2').append('<a class="green" href="'+base_url+'user/campaign/'+json[i].user_id+'/'+campaign_id+'">'+json[i].user_first_name+' '+json[i].user_last_name+'</a>');
