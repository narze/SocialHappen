$(function(){
	for(i in user_companies){
		...
		$.getJSON(base_url+'company/json_get_profile/' + user_companies[i].company_id, function(data) {
			$('.goto div ul').append('<li class="goto-list-company-'+user_companies[i].company_id+'"><p class="thumb"><img src="'+imgsize(data.company_image,'sq')+'" alt="" /></p><h2><a href="'+base_url+'company/'+data.company_id+'">'+data.company_name+'</a></h2></li>');
		});
